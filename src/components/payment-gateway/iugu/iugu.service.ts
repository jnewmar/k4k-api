...
export class IuguService {
    ...
    constructor(private config: ConfigService) {
        this.apiToken = this.config.environment.Iugu.apiToken;
        this.password = this.config.environment.Iugu.password;
        this.urlBase = this.config.environment.Iugu.urlBase;
    ...
    async charge(userId: string, chargeInfo: object): Promise<any> {
        let options = this.optionsTemplate;
        ...
        try {
            payload = await request(options);
            payload['success'] = true;
            if (chargeInfo['no_capture'] == undefined || chargeInfo['no_capture'] == false) {
    ...
    async capture(invoice_id: number): Promise<any> {
    ...
    async removePaymentMethod(userId: string, paymentMethodId: string): Promise<any> {
        let options = this.optionsTemplate;
        options['url'] = `${this.urlBase}customers/${userId}/payment_methods/${paymentMethodId}`;
    ...
    async cancelInvoice(invoice_id: string): Promise<any> {
        let options = this.optionsTemplate;
        options['url'] = `${this.urlBase}invoices/${invoice_id}/cancel`;
        options['method'] = 'PUT';
